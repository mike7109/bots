...
В карточке видно, как у группы устроены рассылки («как общие» / «своё расписание» /
«выключены — только события issue»), а кнопка **«⏱ Рассылки этой группы»** открывает
вкладку «Рассылки» сразу на этой группе.
...
Сверху — переключатель **«Чьи настройки правим»**: **«⚙ Общие»** задают базу для всех
групп, а выбор группы показывает её собственное расписание. У группы есть общий тумблер
**«Рассылки по расписанию»** (выключен = в её комнату идут только события issue по
вебхуку) и свои настройки на каждую карточку: бейдж «своё» значит, что группа
отличается от общего, кнопка **«↺ как общее»** возвращает наследование. Группа хранит
только изменённые поля, поэтому правка общего расписания двигает все группы, которые не
задали своё. Тип уведомления и адрес «Куда» остаются общими — они правятся в «Общих»;
там же живут карточки вебхуков (issue, комментарии, подзадачи). **«▶ Запустить сейчас»**
в режиме группы шлёт только по ней.
...
Правило «теги → комнаты»: задача с любым из перечисленных тегов становится **особой** —
в комнате правила она разворачивается в **тред**: карточка-корень (описание, срок,
исполнитель, метки), а внутрь падают комментарии, смена исполнителя/срока,
закрытие/переоткрытие и статусы подзадач. Обычная комната-источник такого не получает:
комментарии и правки issue идут **только** в комнаты особых задач.

Строка **`*`** в тегах означает «любая задача, даже без меток» — так комната получает
всю жизнь группы, не требуя дисциплины проставления меток. У правила есть **выбор групп**:
ничего не выбрано — правило ловит задачи откуда угодно (как раньше), выбрана группа —
только её. Для `*` это обязательно: без ограничения одна «звёздочка» стянет issue **всех**
групп в свои комнаты, и панель об этом предупреждает.

Комментарии приходят только если в вебхуке GitLab включён триггер **Comments (Note) events**.
