...
Пример в Element — 3 строки (лейблы / статус+исполнители / задача); строка
лейблов исчезает, если их нет. `@` — живой пинг, несколько исполнителей
перечисляются естественно:

> 🏷 чижик 🏷 infra
> 🔴 **Закрыта** · **@Михаил Бахмутский** и **@Никулин Денис**
> #46 **Интеграция вебхука**
>
> 📅 **Дедлайн завтра** (5 июня) · **@Никулин Денис**
> #51 **Выкатить уведомления**
...
Правило в админке («Метки» → «Особые issue») связывает теги с комнатами: задача с таким
тегом разворачивается в **тред** — карточка-корень плюс комментарии, смены исполнителя/срока,
close/reopen и статусы подзадач внутрь. Тег **`*`** = «любая задача», то есть вся группа
без разметки метками; у правила есть ограничение по группам-источникам, и для `*` его нужно
выставлять обязательно, иначе правило заберёт issue всех групп.

Комментарии физически не придут, если в вебхуке GitLab не включён триггер **Comments (Note) events**.

## Рассылки по группам

Источник = GitLab-группа → Matrix-комната. Расписание рассылок читается тремя
слоями: дефолты реестра → общая строка (`settings/pass:<name>`) → своё у группы
(`settings/src:<sid>:pass:<name>`). Группа хранит **только изменённые поля**, поэтому
правка общего расписания двигает всех, кто своего не задал.

У группы есть мастер-тумблер (`settings/src:<sid>:sched`): выключен — в её комнату
идут **только вебхуки** (issue открыта/закрыта/переоткрыта), ни одна рассылка по
расписанию не уходит. Всё настраивается в админке: вкладка «Рассылки» → «Чьи
настройки правим» → выбрать группу (или «⏱ Рассылки этой группы» во вкладке «Группы»).

Общими для всех групп остаются тип уведомления (`rule:<event>`: вкл/выкл и адрес
«Куда») и карточки вебхуков — их правят во вкладке «⚙ Общие».
