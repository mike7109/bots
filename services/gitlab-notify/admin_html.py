...
<div id="app" class="hide"><div class="wrap">
  ...
  <div class="view" data-v="sched">
    <p class="hint" style="margin:0 0 14px">Единое место для всех уведомлений: <b>что</b> слать, <b>куда</b> (Комната / Личка), включён ли тип, <b>когда</b> бот шлёт (дни + время — планирует сам, host-cron не нужен) и разовый запуск. Каждая карточка самодостаточна: триггер (<b>🔔 вебхук</b> — в реальном времени по событию GitLab, <b>⏱ расписание</b> — по дням/времени или интервалу), адрес, выключатель типа, расписание и кнопки «Пример / Предпросмотр / Запустить». Карточка <b>issue</b> (открытие/закрытие) приходит вебхуком — у неё нет расписания и «Запустить сейчас». Дайджесты разделены на «полный» (полная сводка) и «изменения» (дельта со вчера) — это отдельные рассылки со своим расписанием, но <b>один тип</b> (выключатель и адрес общие). Сгруппировано: <b>Групповые</b> → в комнату, <b>Личные</b> → в ЛС. Переключайтесь вкладками ниже.</p>
    <details class="ref"><summary>📖 Справка: кнопки запуска и режимы</summary>
      <div class="refbody">
        ...
        <p><b>Полная сводка и изменения — разные рассылки.</b> Полный дайджест идёт по времени (дни + «Время»). А дайджесты-«изменения» (дельта) идут не по времени, а <b>интервалом</b>: каждые N часов в рабочие часы, или раньше — при всплеске изменений (≥ порога). И только когда есть что показать.</p>
  ...
  <div class="view" data-v="cal">
    ...
    <div class="card"><h2>🔕 Тишина после полного дайджеста</h2>
      <p class="hint">После полной сводки дельта-дайджест («изменения») молчит заданное время, чтобы не повторять только что показанное.</p>
      <div class="row"><span class="mut">Тишина после полного дайджеста, ч (0 — выкл)</span>
        <input type="number" id="quietAfterFull" step="0.5" min="0" style="width:80px">
        <button class="primary sm" onclick="saveQuietAfterFull()">Сохранить</button></div>
    </div>
  </div>
